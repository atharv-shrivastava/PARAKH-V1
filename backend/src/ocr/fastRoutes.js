...
function normalizeEvidence(data) {
  ...
  return {
    evidence,
    rawText: text(data?.result?.rawText) || evidence.map((item) => item.text).join("\n"),
  };
}
...
async function runRapid(images) {
  ...
  const response = await fetch(`${ocrUrl}/api/ocr/analyze`, { method: "POST", body: form });
...
async function runSemanticProviders({ images, rapid, categoryOptions, signal }) {
  ...
  return {
    ...
    timing: Object.fromEntries(settled.map((provider) => [provider.provider, provider.timingMs || 0])),
  };
}

async function analyze(req, res, files) {
  const startedAt = Date.now();
  try {
    ...
    if (submittedBarcode) {
      fields.barcode = {
        ...
        displayValue: submittedBarcode,
        raw: submittedBarcode,
        evidence: submittedBarcode,
        confidence: 1,
        status: "found",
        source: "BARCODE_SCAN",
        verification: "scanner-authoritative",
      };
    }
    ...
    const structured = {
      ...
      suggestedCategory: semantic.suggestedCategory || null,
      warnings: semantic.providerCount < 2
        ? ["Only one semantic AI provider was available; review single-provider findings carefully."]
        : [],
    ...
    const finalResult = await applyEvidenceConfidence(structured, { barcodeImageProvided: false });
    const totalMs = Date.now() - startedAt;
    console.log(`[ocr:fast] images=${files.length} evidence=${rapid.evidence.length} rapid=${rapid.timingMs}ms semantic=${semantic.timingMs}ms gemini=${semantic.timing?.gemini || 0}ms grok=${semantic.timing?.grok || 0}ms providers=${semantic.providerCount} total=${totalMs}ms parallel=true`);
    ...
    return res.json({
      ...
      timing: {
        ...
        semanticMs: semantic.timingMs,
        geminiMs: semantic.timing?.gemini || 0,
        grokMs: semantic.timing?.grok || 0,
        totalMs,
...
router.post("/analyze", authenticate, upload.array("images", config.maxImages), analyze);
