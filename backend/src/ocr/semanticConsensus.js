...
function normalizeQuantityUnit(fields) {
  ...
  next.netQuantity = { ...quantity, value: split.quantity, displayValue: split.quantity, raw: quantity.raw || `${split.quantity} ${split.unit}`, evidence: quantity.evidence || quantity.raw || `${split.quantity} ${split.unit}` };
  if (!unit || !isFound(unit) || !text(unit.value)) next.unit = { ...(unit || {}), value: split.unit, displayValue: split.unit, raw: unit?.raw || `${split.quantity} ${split.unit}`, evidence: unit?.evidence || quantity.evidence || quantity.raw || `${split.quantity} ${split.unit}`, confidence: confidence(unit?.confidence ?? quantity.confidence), status: quantity.status || "found", imageIndex: Number.isInteger(unit?.imageIndex) ? unit.imageIndex : (Number.isInteger(quantity.imageIndex) ? quantity.imageIndex : -1), evidenceIndex: Number.isInteger(unit?.evidenceIndex) ? unit.evidenceIndex : (Number.isInteger(quantity.evidenceIndex) ? quantity.evidenceIndex : -1) };
  return next;
}
...
function voteField(key, providers) {
  const observations = providers.filter((provider) => provider?.enabled && provider?.fields?.[key]).map((provider) => { const fields = normalizeQuantityUnit(provider.fields); const field = sanitizeField(key, fields[key]); return { provider: provider.provider, weight: providerWeight(provider.provider), field, normalized: comparable(field?.value) }; });
  const found = observations.filter((item) => isFound(item.field));
  const votes = observations.map((item) => ({ provider: item.provider, weight: item.weight, status: item.field.status, value: item.field.value ?? null, confidence: confidence(item.field.confidence) }));
  if (!found.length) {
  ...
  if (winning.group.length >= 2 || normalizedWinningVote >= 0.55 || margin >= 0.20) {
    ...
    return { ...best.field, raw: best.field.raw ?? best.field.value, evidence: best.field.evidence ?? best.field.raw ?? best.field.value, verification: `weighted-${winningProviders}`, confidence: Math.max(confidence(best.field.confidence), Math.min(0.98, normalizedWinningVote)), source: "SEMANTIC_CONSENSUS", votes };
  }
  return { value: null, raw: found.map((item) => item.field.raw || item.field.value).filter(Boolean).join(" | ") || null, evidence: found.map((item) => `${item.provider}: ${item.field.evidence || item.field.value}`).join(" | "), confidence: 0, status: "ambiguous", verification: "weighted-conflict", source: "SEMANTIC_CONSENSUS", votes };
}
function voteCategory(providers, categoryOptions) {
  const observations = providers.filter((provider) => provider?.enabled && provider?.suggestedCategory?.categoryId).map((provider) => ({ provider: provider.provider, category: provider.suggestedCategory, id: String(provider.suggestedCategory.categoryId), weight: providerWeight(provider.provider) }));
  ...
  if (winning.length === 1 && enabledCount === 1) return { categoryId: allowed ? String(allowed.id) : winning[0].id, categoryName: allowed ? text(allowed.name) : winning[0].category.categoryName || null, categoryPath: allowed ? text(allowed.path) : winning[0].category.categoryPath || null, confidence: Math.min(confidence(best.category.confidence), 0.79), reason: "Suggested by the available semantic AI provider; verify before registration." };
