...
// Stable imports/constants/state.
...
// Add barcode controls beside the package upload area.
...
// If an older generated integration already contains the parallel flow, repair only missing pieces.
// Otherwise replace only the body of analyze() using brace matching, not a fragile regex.
if (!source.includes("const [ocrOutcome, barcodeOutcome] = await Promise.allSettled")) {
  ...
  if (start >= 0) {
    ...
    console.warn("PARAKH: analyze() not found; leaving existing source unchanged.");
  }
}

// Add an explicit confidence line below every OCR field input.
