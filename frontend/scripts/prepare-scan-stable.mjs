...
if (!source.includes("const [barcodeFile, setBarcodeFile]")) {
  const stateRe = /  const \[message, setMessage\] = useState\(""\);/;
  if (!stateRe.test(source)) throw new Error("Could not locate ScanV2 React state block.");
  source = source.replace(stateRe, (m) => `${m}\n${stateBlock}`);
}

// Repair malformed signatures left by older generated patchers before doing anything else.
source = source.replace(/\bfunction\s+openCamera\(\s*\{/g, "async function openCamera() {");
source = source.replace(/\basync\s+function\s+openCamera\(\s*\{/g, "async function openCamera() {");

if (!source.includes("async function resolveGtin(")) {
  const idx = source.indexOf("function formatElapsed(ms)");
  if (idx < 0) throw new Error("Could not locate formatElapsed in ScanV2.jsx.");
  const helper = `async function resolveGtin(file, manualValue) {
  const manual = String(manualValue || "").replace(/\\s+/g, "").trim();
  if (!file) return { attempted: Boolean(manual), found: Boolean(manual), value: manual || null, gtin: manual || null, format: manual ? "MANUAL_GTIN" : null, confidence: manual ? 0.90 : 0, source: manual ? "MANUAL_GTIN" : "NONE", error: null };
  ...
  const decode = scanBarcodeImage(file).then((decoded) => ({ ...decoded, gtin: decoded.found ? decoded.value : manual || null, source: decoded.found ? "BARCODE_SCAN" : manual ? "MANUAL_GTIN_FALLBACK" : "NONE" }));
...
if (!source.includes("function addBarcodeFile(input)")) {
  const handler = `  function addBarcodeFile(input) {
    const file = input?.[0];
    if (!file) return;
  ...
  const idx = source.search(/\n\s*async function openCamera\(\)\s*\{/);
  if (idx >= 0) {
    source = source.slice(0, idx + 1) + handler + source.slice(idx + 1);
  } else {
    const componentFn = source.search(/\n\s*function\s+(?:closeCamera|capture|remove)\s*\(/);
    if (componentFn < 0) throw new Error("Could not locate a safe component insertion point for barcode handler.");
    source = source.slice(0, componentFn + 1) + handler + source.slice(componentFn + 1);
  }
}
...
if (!source.includes("data-parallel-barcode-ui")) {
  ...
  const limitRe = /<p[^>]*className=["']scan-limit["'][^>]*>[\s\S]*?<\/p>/;
  if (limitRe.test(source)) source = source.replace(limitRe, (m) => `${m}\n      ${ui}`);
  else {
    const headingRe = /(<h2>Capture or upload package images<\/h2>)/;
    if (headingRe.test(source)) source = source.replace(headingRe, (m) => `${m}\n      ${ui}`);
    else source = source.replace(/(<div className="scan-upload-actions">[\s\S]*?<\/div>)/, (m) => `${m}\n      ${ui}`);
  }
}

if (!source.includes("const [ocrOutcome, barcodeOutcome] = await Promise.allSettled")) {
  const re = /  async function analyze\(\)\s*\{[\s\S]*?\n  \}\s*\n\s*function updateOcrField/;
  if (re.test(source)) {
    const fn = `  async function analyze() {
  ...
  function updateOcrField`;
    source = source.replace(re, fn);
  }
}

await fs.writeFile(scanPath, source, "utf8");
console.log("PARAKH stable scan integration applied: barcode UI + preview, barcode/OCR parallel, Rules Engine/DataKart parallel.");
