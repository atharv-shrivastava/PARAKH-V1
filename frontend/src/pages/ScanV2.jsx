...
function formatReferenceValue(value) { if (value == null || value === "") return "—"; if (typeof value === "object") { try { return JSON.stringify(value, null, 2); } catch { return String(value); } } return String(value); }
...
export default function ScanV2() {
  ...
  const resolvedPresentCount = unresolvedFindings.filter((finding) => findingResolutions[finding.findingId]?.decision === "VERIFIED_PRESENT").length;
  const resolvedViolationCount = resolvedViolations.length;
  const resolvedNotApplicableCount = unresolvedFindings.filter((finding) => findingResolutions[finding.findingId]?.decision === "NOT_APPLICABLE").length;
  const baseSummary = compliance?.summary || {};
  const liveComplianceSummary = {
    ...baseSummary,
    passed: Number(baseSummary.passed || 0) + resolvedPresentCount,
    violations: Number(baseSummary.violations || 0) + resolvedViolationCount + manualViolations.length,
    unableToVerify: unresolvedRemaining.length,
    notApplicable: Number(baseSummary.notApplicable || 0) + resolvedNotApplicableCount,
    outOfScope: Number(baseSummary.outOfScope || baseSummary.outOfScopeRules || 0),
  };
  ...
  useEffect(() => {
    const detail = {
      ...
      notApplicable: liveComplianceSummary.notApplicable,
    ...
    liveComplianceSummary.notApplicable,
  ...
  // Rule 23 is assessed after the scan is displayed. Re-run the deterministic
  // Rules Engine immediately when the officer records that assessment so the
  // compliance counters and submission gate reflect the actual decision.
  useEffect(() => {
    if (!ocr) return undefined;

    const handleRule23Assessment = async (event) => {
      const visualFlags = event.detail;
      if (!visualFlags || typeof visualFlags !== "object") return;
      ...
      try {
        ...
        setComplianceError({ message: error?.message || "Rules Engine evaluation failed" });
      }
    };

    window.addEventListener("parakh:rule23-assessment", handleRule23Assessment);
    return () => window.removeEventListener("parakh:rule23-assessment", handleRule23Assessment);
  }, [ocr]);
  ...
  async function analyze() {
    ...
    setAnalyzing(true); setAnalysisDurationMs(null); setCompliance(null); setComplianceError(null); setVerificationConfidence(null); setDatakartVerification({ ...EMPTY_DATAKART }); window.sessionStorage.removeItem("parakhRule23Assessment");
    ...
    try {
      ...
      setOcr(extracted); setForm(formFromOcr(extracted, authoritativeGtin || "")); setUseExtractedData(true); setShowRegistration(true); setAiSuggestedCategory(
        info.aiSuggestedCategory ||
        extracted.aiSemantic?.suggestedCategory ||
        extracted.suggestedCategory ||
        null,
      ); setBarcodeResult(identifier); setProviderInfo({ ...info, barcodeResult: identifier });
