...
function Reports() {
  const [products, setProducts] = useState([]); const [selectedProductId, setSelectedProductId] = useState(() => new URLSearchParams(window.location.search).get("productId") || ""); const [query, setQuery] = useState(""); const [status, setStatus] = useState("ALL"); const [sourceType, setSourceType] = useState("ALL"); const [brand, setBrand] = useState(""); const [shop, setShop] = useState(""); const [dateFrom, setDateFrom] = useState(""); const [dateTo, setDateTo] = useState(""); const [error, setError] = useState(""); const [loading, setLoading] = useState(true); const [activeRules, setActiveRules] = useState([]); const user = getUser();
  ...
  const shown = useMemo(() => { const search = query.trim().toLowerCase(), brandSearch = brand.trim().toLowerCase(), shopSearch = shop.trim().toLowerCase(); return products.filter((product) => { if (selectedProductId && String(product.id) !== String(selectedProductId)) return false; const inspection = product.inspections?.[0]; const inspectedAt = new Date(inspection?.inspectedAt || product.createdAt); const sourceName = product.sourceWebsiteName || inspection?.shop?.name || ""; const searchable = [product.productName, product.brandName, product.category?.name, inspection?.shop?.name, product.sourceWebsiteName, product.barcode].filter(Boolean).join(" ").toLowerCase(); return (!search || searchable.includes(search)) && (status === "ALL" || product.complianceStatus === status) && (!brandSearch || (product.brandName || "").toLowerCase().includes(brandSearch)) && (!shopSearch || sourceName.toLowerCase().includes(shopSearch)) && (!dateFrom || inspectedAt >= new Date(`${dateFrom}T00:00:00`)) && (!dateTo || inspectedAt <= new Date(`${dateTo}T23:59:59.999`)); }); }, [products, query, status, brand, shop, dateFrom, dateTo, selectedProductId]);
  function clearFilters() { setQuery(""); setStatus("ALL"); setSourceType("ALL"); setBrand(""); setShop(""); setDateFrom(""); setDateTo(""); setSelectedProductId(""); window.history.replaceState({}, "", "/reports"); }
