...
function verificationBadge(field) {
  const verification = String(field?.verification || "").toUpperCase();
  if (verification === "MATCH" || field?.verificationIcon === "✓") return { icon: "match", label: "MATCH", tone: "#16a34a", background: "#dcfce7", border: "#86efac" };
  if (verification === "MISMATCH" || field?.verificationIcon === "✕") return { icon: "mismatch", label: "MISMATCH", tone: "#dc2626", background: "#fee2e2", border: "#fca5a5" };
  return { icon: "unverified", label: "UNVERIFIED", tone: "#a16207", background: "#fef3c7", border: "#fcd34d" };
}

function VerificationIcon({ type }) {
  if (type === "match") {
    return <svg aria-hidden="true" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="m5 12 4 4L19 6" /></svg>;
  }
  if (type === "mismatch") {
    return <svg aria-hidden="true" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round"><path d="m7 7 10 10M17 7 7 17" /></svg>;
  }
  return <svg aria-hidden="true" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round"><circle cx="12" cy="12" r="9" /><path d="M12 10v6M12 7h.01" /></svg>;
}
...
function Scan() {
  ...
  async function analyzeImages() {
    ...
    try {
      ...
      setShowRegistration(false);
      setUseExtractedData(false);
      setMessage(data.complianceError ? `OCR complete. Rules Engine unavailable: ${data.complianceError.message}` : "OCR and Rules Engine analysis complete. Review the extracted data, then register the product.");
    } catch (e) {
      setMessage(e.message || "OCR analysis failed.");
    } finally {
