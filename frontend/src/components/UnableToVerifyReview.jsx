...
function installComplianceCapture() {
  if (installed || typeof window === "undefined") return;
  installed = true;
  capturedCompliance = readJson(CAPTURED_KEY, null);
  activeDecisions = readJson(DECISIONS_KEY, {});
  ...
  window.fetch = async (input, init = {}) => {
    ...
    if (method === "POST" && /\/api\/ocr\/evaluate-structured(?:\?|$)/.test(url)) {
      ...
      try {
        ...
        if (compliance) {
          capturedCompliance = compliance;
          activeDecisions = {};
          window.sessionStorage.setItem(CAPTURED_KEY, JSON.stringify(compliance));
          window.sessionStorage.removeItem(DECISIONS_KEY);
          window.dispatchEvent(new CustomEvent(REVIEW_EVENT, { detail: compliance }));
        }
      } catch {}
      return response;
    }
    ...
    if (method === "POST" && /\/api\/products(?:\?|$)/.test(url)) {
      ...
      const compliance = body?.ocrData?.compliance;
      ...
      const failIds = unable.filter((finding) => activeDecisions[finding.findingId] === "FAIL").map((finding) => finding.findingId);
      const passIds = unable.filter((finding) => activeDecisions[finding.findingId] === "PASS").map((finding) => finding.findingId);
      const failSet = new Set(failIds);
      const passSet = new Set(passIds);
      const originalSummary = compliance?.summary || {};
...
installComplianceCapture();

function setDecisionState(next) {
  activeDecisions = next;
  window.sessionStorage.setItem(DECISIONS_KEY, JSON.stringify(activeDecisions));
...
export default function UnableToVerifyReview() {
  ...
  useEffect(() => {
    const handleCompliance = (event) => {
      const nextCompliance = event.detail || null;
      capturedCompliance = nextCompliance;
      setCompliance(nextCompliance);
      setDecisions({ ...activeDecisions });
    };
    window.addEventListener(REVIEW_EVENT, handleCompliance);
    return () => window.removeEventListener(REVIEW_EVENT, handleCompliance);
  }, []);
