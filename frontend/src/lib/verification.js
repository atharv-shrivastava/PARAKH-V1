...
function fieldScore(aiField, referenceValue, key) {
  ...
  return normalize(aiField.value) === normalize(referenceValue) ? 1 : normalize(aiField.value).includes(normalize(referenceValue)) || normalize(referenceValue).includes(normalize(aiField.value)) ? 0.85 : 0;
}
...
export async function scanBarcodeImage(file) {
  ...
  if (!("BarcodeDetector" in window)) {
    return { attempted: true, found: false, value: null, format: null, confidence: 0, error: "This browser does not provide BarcodeDetector." };
  }
  try {
    ...
    bitmap.close();
    const result = results.find((item) => /^\d{8,18}$/.test(String(item.rawValue || ""))) || results[0];
    if (!result?.rawValue) return { attempted: true, found: false, value: null, format: null, confidence: 0, error: "No readable barcode was found in the uploaded image." };
    return { attempted: true, found: true, value: String(result.rawValue), format: result.format || null, confidence: 0.98, error: null };
  } catch (error) {
...
export async function lookupDataKart(gtin, signal) {
  ...
  try {
    const response = await fetch(`${DATAKART_API_URL}/api/products/gtin/${encodeURIComponent(normalizedGtin)}`, {
      signal,
...
export function compareWithDataKart(ocrResult, dataKart) {
  ...
  for (const [key, column] of Object.entries(FIELD_MAP)) {
    ...
    comparisons[key] = {
      aiValue: ocrResult[key].value,
      referenceValue: dataKart.product[column],
      score,
      match: score >= 0.85,
    };
  }
  ...
  return {
    matchedFields: values.filter((item) => item.match).length,
    comparedFields: values.length,
    matchRate: values.length ? values.reduce((sum, item) => sum + item.score, 0) / values.length : null,
    comparisons,
  };
}
...
export function calculateVerificationConfidence({ ocrResult, providerInfo, barcodeResult, dataKartComparison }) {
  const ocrEvidence = Array.isArray(ocrResult?.rawOcrEvidence) ? ocrResult.rawOcrEvidence.map((item) => Number(item.confidence)).filter(Number.isFinite) : [];
  const ocrConfidence = average(ocrEvidence);
  ...
  const providerCount = Number(providerInfo?.semantic?.providerCount || providerInfo?.semantic?.providers?.length || 0);
  ...
  return {
    ...
    components,
    providerCount,
    disclaimer: "This is an evidence-confidence score for the verification pipeline, not a statistical probability of legal compliance.",
  };
}
